      ...
      >
        <div className="flex flex-row justify-between items-center w-full">
          ...
          {open ? (
            ...
            <IconMenu2 className="text-white z-10" onClick={() => setOpen(!open)} />
          )}
        </div>
...
const MobileChildNavItems = ({ navItem }: { navItem: any }) => {
  ...
  return (
    <motion.div className="overflow-hidden">
      <button onClick={() => setOpen(!open)} className="relative text-white flex w-full justify-between">
        <motion.span className="block">{navItem.name}</motion.span>
        ...
        {open && (
          ...
          >
            {navItem.children.map((child: any, childIdx: number) => (
              <Link key={`child-${childIdx}`} href={child.link} className="relative text-white">
