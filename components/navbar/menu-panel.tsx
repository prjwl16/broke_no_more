...
const SectionTitle: React.FC<{
  ...
  <h2 className={cn("text-xs text-neutral font-semibold mb-4", isFirst && "mb-8")}>{title}</h2>
)
...
const SectionContent: React.FC<{
  ...
  <div className="space-y-6">
    {features.map((feature, index) => {
      ...
      return (
        <Link href={feature.url} key={index} className="flex items-start space-x-4 group">
          {isIcon && Icon && (
          ...
          <div>
            ...
            <p className="text-xs text-neutral group-hover:text-white">{description}</p>
          </div>
        </Link>
      )
    })}
      ...
      <div className="col-span-6">
        {primaryFeatures.map((primaryFeature, index) => (
          <div key={index} className="p-4.5 pr-0">
            ...
            {showExploreLink && (
              <div className="mt-8">
                <Link href="#" className="text-sm flex items-center text-link hover:underline">
        ...
        <div className="col-span-5 bg-tertiary-accent">
          {secondaryFeatures.map((secondaryFeature, index) => (
            <div key={index} className="pl-4.5 pt-4.5">
              ...
              <SectionContent features={secondaryFeature.features} isIcon={true} />
            </div>
          ))}
        </div>
      )}
        ...
        <div className="col-span-5 h-full bg-tertiary-accent p-4.5">
          ...
          <Link href={"#"} className="grid grid-rows-3 gap-4 mt-5">
            {secondaryCards.features.map((secondaryCard, index) => (
              <div key={index} className="p-4 pr-0 border border-surface-overlay rounded-lg">
                <h3 className="text-xs text-white mb-4">{secondaryCard.title}</h3>
                <div className="flex items-end justify-between pr-4">
                  <p className="text-xs mt-4 text-neutral max-w-36">{secondaryCard.description}</p>
                  <MoveRight size={16} className="text-white hover:text-primary-accent" />
                </div>
              </div>
            ))}
          </Link>
